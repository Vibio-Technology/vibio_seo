...
优先使用 `scripts/state_manager.py` 管理 `.vibio/state/` 的机器真源；`project.md`、`changelog.md` 等 Markdown 仅作为兼容或派生的人读视图。工具提供 `init`、`append-change`、`append-review`、`validate`、`render` 和 `detectability`，用追加式 SHA-256 链保存历史，阻止非法状态跳跃，并拒绝电子邮箱、手机号和常见凭据。

```bash
python scripts/state_manager.py init --project-root . \
  --project-id example-seo --site-url https://example.com/ \
  --market DE --language de-DE
python scripts/state_manager.py validate --project-root .
python scripts/state_manager.py render --project-root . --out .vibio/project.md
```

只有用户授权项目内写入时才能初始化或追加。实验变更的首条 `planned` 记录必须在实施前用 `experiment_registration` 绑定 experiment ID、项目内 plan、plan 文件 SHA-256 与 plan hash；普通 planned 记录不能事后升级成“预注册”，需要另建 change ID。构建通过最多进入 `artifact_verified`；`incremental-positive` 与 `no-detectable-change` 都必须引用同一冻结 `experiment_plan`、完整 `experiment_result` 及结果文件 SHA-256，并通过 `experiment_inputs` 绑定 panel、artifact report 和可选 measurement metadata 的项目内路径/SHA-256。工具会重跑分析，并比对除 `analyzed_at` 外的整份报告，包括拒绝额外字段，因此不接受只有内部一致性的自报结果。前者还要通过预注册方向、置信区间和全部 guardrail 门禁；后者必须引用 `state_manager.py detectability` 生成且带 SHA-256 的 JSON，并在写入时按同一 experiment ID、plan hash、MDE、alpha、标准误与置信区间确定性重算。不要绕过工具直接编辑 JSONL hash 链。

页面级受控实验使用 `scripts/experiment.py` 保存可复验的决策链：PLAN 阶段用 `plan` 冻结 spec、基线来源 hash、逐页面基线摘要、seed、分组、指标、护栏、窗口与 MDE，加载时从冻结基线重算分组和 balance；REVIEW 阶段用 `analyze` 校验 plan hash、真实产物、日粒度窗口覆盖、逐页面冻结基线、panel、污染和估计结果。`schemas/experiment.plan.schema.json` 与 `schemas/experiment.report.schema.json` 约束结构，标准库语义校验器另行重算 arms、DiD、CI、guardrail、coverage 和 MDE 精度，不把“Schema 有效”等同于结果真实。

MEMORY 不把工具资格状态改写成业务结论。`eligible_incremental` 只能记录为“可进入人工增量评估”；写入 `incremental-positive` 仍需保留方向、区间、反事实假设、机制、适用范围和人工判定。`no-detectable-change` 只在确定性证据显示 power >= 0.8、正式报告置信区间包含 0，且所有实施、覆盖、污染、guardrail 与测量门禁通过时才能写入；它不证明效果绝对为零。实验单位只使用页面或匹配页面对，不把用户、邮箱、线索 ID 或原始 CRM 行写入 spec、baseline、panel 或状态。
...
| REVIEW | 比较窗口、实验/反事实方法、证据强度、判定与下一决策 |
