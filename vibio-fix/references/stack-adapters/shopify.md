...
### robots.txt
Edit `templates/robots.txt.liquid` to add rules (e.g. disallow `/cart`, `/checkout`, internal search `/search`, tag filters) while keeping Shopify's defaults via `{{ robots.default_rules }}`.
...
### llms.txt
No native route. Add `templates/llms.txt.liquid` won't work (Shopify only allows the robots template). Use an app or a reverse-proxy/edge worker, or host `llms.txt` on the apex via the CDN if available. If none possible, note the limitation.
