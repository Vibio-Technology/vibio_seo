...
> 第一次使用请从 [Vibio SEO v5 中文使用教程](docs/getting-started.zh-CN.md) 开始。教程包含安装、5 分钟离线验收、真实项目提示词、八种模式选择、六个运行工具、权限与隐私边界、效果复盘和常见问题排查。
...
每项实质性任务都应交付：资源焦点与独立严重阻断的证据、已实施变更或决策、当前验证结果、后续指标与观察窗口、风险和缺失数据，以及接下来三项行动。只有用户明确授权项目内状态写入时，才更新 `.vibio/`。
...
最小安装只需 `vibio/`，其 Skill 名称是 `vibio-seo`，且构建后包含完整参考包。普通 SEO 用户推荐同时安装七个专项模式 Skill 和 `vibio-memory`，以获得更准确的直接触发和项目状态支持：
...
`vibio-factory` 只用于创建或维护 Vibio 专项 Skill；需要该能力的维护者再单独安装：
...
也可以明确调用专项 Skill，例如 `vibio-keyword`、`vibio-content` 或 `vibio-link`。所有模式在开工前读取已有的项目根 `.vibio/`；只有用户授权状态写入时，收工后才写回会影响后续决策的状态。
