...
### seo-image-gen
AI image generation for SEO assets: OG/social-preview images, blog hero images, schema images, product photography, infographics. Powered by Gemini via nanobanana-mcp (requires `banana` extension). Fire for "generate image", "OG image", "social preview", "hero image". Pairs naturally with the OG-image fix in `nextjs-playbook.md` when no suitable landscape image exists.
...
- **Codebase SEO review** → manual checklist in `nextjs-playbook.md` first, then `seo-schema` to validate JSON-LD and `seo-technical` for crawl/index logic.
...
- **AI-search visibility** → `seo-geo` + the `llms.txt` recipe in `nextjs-playbook.md`.
